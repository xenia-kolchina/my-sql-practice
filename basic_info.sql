...
-- ==========================================================
-- ШПАРГАЛКА ПО СТРУКТУРЕ SQL ЗАПРОСА (С АГРЕГАЦИЕЙ И JOIN)
-- ==========================================================
/* 
ПОРЯДОК НАПИСАНИЯ (как мы пишем код):
1. SELECT   [колонки или SUM/AVG/COUNT]
2. FROM     [таблица 1]
3. JOIN     [таблица 2] ON [условие связи]
4. WHERE    [фильтрация строк до расчетов]
5. GROUP BY [группировка по неагрегированным колонкам]
6. HAVING   [фильтрация после расчетов (на результат SUM/COUNT)]
7. ORDER BY [сортировка итога]
8. LIMIT    [ограничение количества строк]
*/
SELECT 
    m.member_name, 
    SUM(p.price * p.amount) AS total_spent  -- Перемножение и агрегация
FROM FamilyMembers AS m
JOIN Payments AS p ON m.member_id = p.family_member_id
WHERE 
    p.date BETWEEN '2023-01-01' AND '2023-12-31' -- Фильтр сырых данных (период)
    AND p.category != 'Salary'
GROUP BY 
    m.member_name                                -- Группируем по имени
HAVING 
    SUM(p.price * p.amount) > 1000               -- Фильтр посчитанной суммы
ORDER BY 
    total_spent DESC;                            -- Сортировка от больших трат к меньшим
/* 
ЛОГИЧЕСКИЙ ПОРЯДОК ВЫПОЛНЕНИЯ (как SQL видит запрос):
1. FROM & JOIN : Собирает данные из таблиц в одну "кучу".
2. WHERE       : Выкидывает лишние строки (например, другие даты).
3. GROUP BY    : Раскладывает оставшиеся строки по "папкам" (группам).
4. Агрегация   : Внутри каждой "папки" считает сумму (SUM) или среднее.
5. HAVING      : Выкидывает целые "папки", которые не подошли по сумме.
6. SELECT      : Выбирает, какие колонки показать на экране.
7. ORDER BY    : Расставляет результат по порядку.
*/
...
/* если в условии задачи просят вывести данные, которых может не быть (нули, пустые значения), это почти всегда сигнал использовать LEFT JOIN.
Обычный JOIN (или INNER JOIN) просто отрезает все строки, для которых нет пары в другой таблице, и ваши «пустые» рейсы просто исчезли бы из отчета. */
/* объединение сначала двух, а потом с третьей таблиц делается с помощью последовательных операторов JOIN. 
Вы просто «паровозиком» прицепляете таблицы одну за другой. Просто JOIN = INNER JOIN */
-- ==========================================================
